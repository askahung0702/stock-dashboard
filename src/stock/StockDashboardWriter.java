...
public class StockDashboardWriter {
    ...
    private void appendStyles(StringBuilder builder) {
        ...
        builder.append(
                ".chart-card svg{width:100%;height:220px;display:block;background:rgba(255,255,255,.66);border-radius:14px;} .chart-legend{display:flex;gap:10px;flex-wrap:wrap;margin:0 0 8px;font-size:12px;color:#6c655c}.chart-legend span{display:inline-flex;align-items:center;gap:5px}.chart-legend i{width:18px;height:3px;border-radius:999px;display:inline-block} ");
        builder.append(
    ...
    private void appendHistoryExplorerSection(StringBuilder builder, HistoryBundle historyBundle) {
        builder.append("<section class=\"panel\">");
        builder.append("<div class=\"section-head\"><div><h2>個股歷史追蹤</h2><p class=\"hint\">輸入代碼或名稱，查看這檔股票每天的分數、股價、短天期均線、量比與 Likely 狀態變化。</p></div></div>");
        builder.append("<div class=\"toolbar\">");
        ...
        builder.append("<article class=\"chart-card\"><h3>股價 / 均線</h3><svg id=\"history-price-chart\"></svg></article>");
    ...
    private String buildDashboardScript() {
        ...
        builder.append("function chartValues(history,key){return history.map(function(item){var value=Number(item[key]||0);return isFinite(value)?value:null;});}");
        builder.append("function rollingAverageValues(history,key,days){var source=chartValues(history,key);var values=[];for(var i=0;i<source.length;i++){if(i+1<days){values.push(null);continue;}var total=0;var count=0;for(var j=i-days+1;j<=i;j++){var value=source[j];if(value!==null){total+=value;count+=1;}}values.push(count===days?total/days:null);}return values;}");
        builder.append("function drawMultiChart(elementId,history,series){var element=document.getElementById(elementId);if(!element){return;}if(!history||!history.length){element.innerHTML='';return;}var width=640;var height=220;var left=34;var right=28;var bottom=28;var hasLegend=series.some(function(item){return item.label;});var top=hasLegend?42:28;var all=[];series.forEach(function(item){item.values.forEach(function(value){if(value!==null&&isFinite(value)){all.push(value);}});});if(!all.length){element.innerHTML='';return;}var min=Math.min.apply(null,all);var max=Math.max.apply(null,all);if(min===max){min=min-1;max=max+1;}var chartHeight=height-top-bottom;var chartWidth=width-left-right;var stepX=history.length===1?0:chartWidth/(history.length-1);function xAt(index){return history.length===1?left+chartWidth/2:left+stepX*index;}function yAt(value){return height-bottom-((value-min)/(max-min))*chartHeight;}var firstDate=dateLabel(history[0].date);var lastDate=dateLabel(history[history.length-1].date);var svg='';svg+='<svg viewBox=\"0 0 '+width+' '+height+'\" preserveAspectRatio=\"none\">';svg+='<line x1=\"'+left+'\" y1=\"'+(height-bottom)+'\" x2=\"'+(width-right)+'\" y2=\"'+(height-bottom)+'\" stroke=\"rgba(95,76,54,.20)\" stroke-width=\"1\" />';svg+='<line x1=\"'+left+'\" y1=\"'+top+'\" x2=\"'+left+'\" y2=\"'+(height-bottom)+'\" stroke=\"rgba(95,76,54,.20)\" stroke-width=\"1\" />';if(hasLegend){var legendX=left;series.forEach(function(item){if(!item.label){return;}svg+='<line x1=\"'+legendX+'\" y1=\"16\" x2=\"'+(legendX+18)+'\" y2=\"16\" stroke=\"'+item.color+'\" stroke-width=\"'+(item.width||2)+'\" />';svg+='<text x=\"'+(legendX+24)+'\" y=\"20\" fill=\"#6c655c\" font-size=\"12\">'+item.label+'</text>';legendX+=item.label.length*10+48;});}series.forEach(function(item){var points=[];var lastPoint=null;for(var i=0;i<item.values.length;i++){var value=item.values[i];if(value===null||!isFinite(value)){continue;}var point=xAt(i).toFixed(1)+','+yAt(value).toFixed(1);points.push(point);lastPoint=point;}if(points.length){svg+='<polyline fill=\"none\" stroke=\"'+item.color+'\" stroke-width=\"'+(item.width||2)+'\" stroke-linecap=\"round\" stroke-linejoin=\"round\" points=\"'+points.join(' ')+'\" />';if(lastPoint){var xy=lastPoint.split(',');svg+='<circle cx=\"'+xy[0]+'\" cy=\"'+xy[1]+'\" r=\"3.5\" fill=\"'+item.color+'\" />';}}});svg+='<text x=\"'+left+'\" y=\"'+(height-8)+'\" fill=\"#6c655c\" font-size=\"12\">'+firstDate+'</text>';svg+='<text x=\"'+(width-right-74)+'\" y=\"'+(height-8)+'\" fill=\"#6c655c\" font-size=\"12\">'+lastDate+'</text>';svg+='<text x=\"'+left+'\" y=\"'+(top-8)+'\" fill=\"#6c655c\" font-size=\"12\">'+fmt(max,2)+'</text>';svg+='<text x=\"'+left+'\" y=\"'+(height-bottom+18)+'\" fill=\"#6c655c\" font-size=\"12\">'+fmt(min,2)+'</text>';svg+='</svg>';element.innerHTML=svg;}");
        builder.append("function drawChart(elementId,history,key,color){drawMultiChart(elementId,history,[{label:'',color:color,width:3,values:chartValues(history,key)}]);}");
        builder.append("function drawPriceMaChart(elementId,history){drawMultiChart(elementId,history,[{label:'股價',color:'#1d5f74',width:3,values:chartValues(history,'price')},{label:'MA5',color:'#ba5a31',width:2,values:rollingAverageValues(history,'price',5)},{label:'MA10',color:'#7a5ac8',width:2,values:rollingAverageValues(history,'price',10)},{label:'MA18',color:'#317a46',width:2,values:rollingAverageValues(history,'price',18)}]);}");
