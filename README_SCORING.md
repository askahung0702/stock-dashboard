...
## 2026-09-16 市場資料品質與風控修正

- 所有大盤核心資料必須與目前快照交易日一致；過期期貨價格不再參與期貨風控分數。
- 台指期分鐘線若不是本次交易日，抓取階段直接標記 unavailable，排程不再靜默沿用歷史舊值。
- 加權指數改以 Yahoo 日線中「不晚於快照日且日期完全相符」的收盤資料計算，不再混用即時 meta 價格。
- 前端將「市場資料品質」與「個股資料完整度」分開顯示；缺少指數日期或品質低於門檻時暫停新的資金水位、策略模式與 ATR 建議。
- 外資期貨明確拆成空單總量與多空淨部位，避免將 94,086 口空單誤讀為淨空單。
- 趨勢惡化分數是規則分數、不是機率；站上與跌破 MA20 為互補指標，不重複加權。
- 恐慌盤改為 `RiskOff`，不再推薦 `highWinMode`；ATR 僅為風險距離參考，波動升高時以縮小股數控制單筆風險。
...
定位為「早期可研究的共振」，不是噴出後的追價清單。2026/04/25 → 2026/06/08
近期大漲樣本顯示，營收、EPS、20 日報酬剛轉正與未過熱比單日籌碼更穩定，
因此籌碼改列輔助條件，不再一票否決。

核心條件：

- 最新月營收年增 > 0，近 3 月平均年增 > 8，且近 3 月正成長月數 >= 2
- EPS/獲利有支撐
- 現價站上 MA20，且 MA20 >= MA60
- 20 日漲幅介於 1 到 25，60 日漲幅介於 -15 到 60
- 未過熱：20 日漲幅 <= 25、量比 <= 2.5、距 60 日高點回檔 -25 到 -2，RSI < 76

輔助條件，至少符合 4 項：

- 5 日法人未明顯轉弱、單日法人轉買，或主力站買方
- MA60 > MA120
- 量比介於 0.8 到 2.5
- 結構為回踩承接、整理待確認或平台突破
- 估值不極端
